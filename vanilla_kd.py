...
def main_worker(gpu, ngpus_per_node, args):
    ...
    log_name = 'R%d-%s-%s-%s' % (args.rank, args.dataset, args.teacher,
                                 args.student) if args.multiprocessing_distributed else '%s-%s-%s' % (args.dataset,
                                                                                                      args.teacher,
                                                                                                      args.student)
    args.logger = datafree.utils.logger.get_logger(
        log_name, output='checkpoints/vanilla_kd/log-%s-%s-%s-%s%s.txt' % (args.dataset, args.transfer_set,
                                                                           args.teacher, args.student, args.log_tag))
    if args.rank <= 0:
    ...
    if os.path.isdir(args.transfer_set):
        ...
        _, train_dataset, _ = registry.get_dataset(name=args.transfer_set, data_root=args.data_root,
                                                   test_split=args.split, independent_dataset=args.independent_dataset)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, [student, teacher], optimizer, args, num_classes)
        student.eval()
        eval_results = evaluator(student, device=args.gpu)
        (acc1, acc5), val_loss = eval_results['Acc'], eval_results['Loss']
        ...
        _best_ckpt = 'checkpoints/vanilla_kd/%s-%s-%s-%s.pth' % (args.dataset,
                                                                 args.transfer_set, args.teacher, args.student)
...
def train(train_loader, model, optimizer, args, num_classes):
